class SectorsController < ApplicationController
  ...
  before_action :get_services, only: [:new, :edit, :create]

  # GET /sectors or /sectors.json
  def index
    @sectors = Sector.all.order(:service, :name)
  end
  ...
  private
    ...
    def sector_params
      params.require(:sector).permit(:name, :service_id)
    end

    def get_services
      @services = Service.all.order(:name, :floor)
    end
end
